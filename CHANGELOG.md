...
- `@zephyr/cloud-sdk` typed search methods:
  - `searchIcons`
  - `listAvatarStyles`
  - `searchLogos`
- `@zephyr/cloud-sdk` license validation method:
  - `validateLicense`
...
- `@zephyr/core`: token system, config model with 6 style packs, CSS variable generation
- `@zephyr/ui-react`: 40+ components — Button, Input, Avatar, DataTable, SearchResultsPanel, CommandBar, and more
...
- `@zephyr/ai-registry`: machine-readable component registry with props schema, a11y notes, and AI hints
- `@zephyr/mcp-server`: MCP tools — `search_components`, `get_component_spec`, `generate_component`, `scaffold_page`, `apply_theme`, `list_templates`
