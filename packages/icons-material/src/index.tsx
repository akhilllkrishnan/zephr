...
function defineIcons(names: string[], category: string, keywords: string[]): MaterialIconDefinition[] {
...
const iconCatalog: MaterialIconDefinition[] = [
  ...defineIcons(
    [
      "home",
      "menu",
    ...
    "navigation",
    ["navigation", "layout", "ui", "menu", "panel"]
  ),
  ...defineIcons(
    [
      "person",
    ...
    "identity",
    ["identity", "user", "people", "profile", "account"]
  ),
  ...defineIcons(
    [
      "notifications",
    ...
    "status",
    ["status", "alert", "notification", "feedback"]
  ),
  ...defineIcons(
    [
      "mail",
    ...
    "communication",
    ["communication", "message", "chat", "email", "support"]
  ),
  ...defineIcons(
    [
      "description",
    ...
    "files",
    ["files", "documents", "editor", "storage", "content"]
  ),
  ...defineIcons(
    [
      "lock",
    ...
    "security",
    ["security", "privacy", "protection", "auth", "access"]
  ),
  ...defineIcons(
    [
      "shopping_cart",
    ...
    "commerce",
    ["commerce", "payment", "checkout", "billing", "finance"]
  ),
  ...defineIcons(
    [
      "bar_chart",
    ...
    "analytics",
    ["analytics", "metrics", "charts", "dashboard", "data"]
  ),
  ...defineIcons(
    [
      "smart_toy",
    ...
    "developer",
    ["developer", "ai", "automation", "cloud", "platform"]
  ),
  ...defineIcons(
    [
      "image",
    ...
    "media",
    ["media", "creative", "design", "content", "visual"]
  ),
  ...defineIcons(
    [
      "calendar_today",
    ...
    "time",
    ["time", "schedule", "workflow", "task", "calendar"]
  ),
  ...defineIcons(
    [
      "flight",
    ...
    "location",
    ["location", "maps", "travel", "logistics", "geo"]
  ),
  ...defineIcons(
    [
      "favorite",
    ...
    "engagement",
    ["engagement", "rating", "reaction", "social", "feedback"]
  )
];

const catalogByName = new Map(iconCatalog.map((icon) => [icon.name, icon]));

export const MATERIAL_ICON_STYLES: MaterialIconStyle[] = [
  "filled",
  "outlined",
  "rounded",
  "sharp"
...
export function normalizeMaterialIconName(value: string): string {
  return value.trim().toLowerCase().replace(/[\s-]+/g, "_").replace(/[^a-z0-9_]/g, "");
}

export function listMaterialIcons(): MaterialIconDefinition[] {
  return [...iconCatalog];
}
...
function scoreIcon(icon: MaterialIconDefinition, tokens: string[], normalizedQuery: string): number {
...
export function searchMaterialIcons(query: string, options: SearchMaterialIconsOptions = {}): MaterialIconDefinition[] {
...
export function getMaterialIcon(name: string): MaterialIconDefinition | null {
...
const familyByStyle: Record<MaterialIconStyle, string> = {
